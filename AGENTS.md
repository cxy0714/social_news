...
produced by Codex itself (web search/fetch) on a cloud schedule and committed to
...
1. **Cloud scheduled task (primary).** Codex runs on Anthropic's cloud nightly,
...
2. **Local helper.** `scripts/fetch_news.py` runs on a user's own machine (no
   cloud egress restrictions) to pre-fetch RSS candidates. Codex then classifies
...
- Output `digests/_raw-*.md` is a gitignored intermediate; the real deliverable
  is still `digests/YYYY-MM-DD.md`, which Codex writes from these candidates.
...
- Cloud runs may be limited to `Codex/`-prefixed branches unless the repo has
