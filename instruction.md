...
6. **概念观察（固定栏目）**：面向「学习宏观经济学与社会学」的读者，从当日新闻里抽取约 3 个宏观经济学概念 + 约 3 个社会学概念。每个概念写「**中英文名称** — 一句定义。📰 对应：当日哪条新闻 + 为何能挂上这个理论」。要从真实条目出发，不要硬套；概念可随当日新闻自然变化。
7. 写文件 `digests/YYYY-MM-DD.md`（格式见第 3 节，**按类型分区 + 概念观察栏目**），并更新 `README.md` 顶部的「最近 7 天」索引。
8. `git add . && git commit -m "news: YYYY-MM-DD" && git push`。
...
- **去重（合并而非丢弃）**：同一事件多源时合并为一条，但**列出所有报道该事件的媒体链接**，另起一行 `📎 报道(N)：[媒体A](url) · [媒体B](url) · …`。主摘要选信息最全者撰写；不要只留一个链接而丢掉其他家。
...
## 政治 · 国际
- **事件中文小标题** — 中文一句话摘要。`中东`
  📎 报道(3)：[Gulf News](https://...) · [Al Jazeera](https://...) · [BBC](https://...)
...
### 宏观经济学
- **概念名（English term）** — 一句定义。📰 对应：当日某条新闻 + 为何挂得上。
...
> 各媒体的**类型 / 政治立场 / 专业领域 / 是否付费墙**见 [`sources.md`](./sources.md)（固定参考，判断来源属性用）。
