# Daily Social News Digest — Instruction for Claude Code（云端定时任务版）

> 本文件是给 **Claude Code 云端定时任务（Cloud Scheduled Task / Routine）** 的运行说明。
> 形态：每天凌晨在 Anthropic 云端自动运行（Max 可用，电脑关机也跑）——用 Claude **自带的网页搜索/抓取能力**巡视主流报纸，挑出**社会新闻**，写中文摘要，提交到本（私有）仓库。
...
3. 判定是否属「社会新闻」（公共事件、教育、公共卫生、民生、社会现象、灾害/突发、人物/人间故事、社会治理等；非纯政治博弈/财经行情/体育）。每区域保留约 5–10 条高价值条目并**去重**。
4. 每条用**自己的话**写 1–2 句中文摘要（≤ 50 字），保留原标题（可附中文译题），附原文链接。
5. 打标签：region（北美/欧洲/中国大陆/港澳台/亚太/中东其他）+ category（突发/民生/公共卫生/教育/社会现象/人物/灾害/其他）。
6. 写文件 `digests/YYYY-MM-DD.md`（格式见第 3 节），并更新 `README.md` 顶部的「最近 7 天」索引。
...
## 北美
- **[原标题](https://...)** — 中文一句话摘要。`突发` · BBC
- **[原标题](https://...)** — 中文一句话摘要。`民生` · NYT
...
```
读取仓库根目录的 instruction.md，按其中“每日工作流”为今天（北京时间）生成一份社会新闻摘要：
用你的网页搜索/抓取能力巡视 instruction.md 列出的来源，筛选并去重，写入 digests/YYYY-MM-DD.md，
